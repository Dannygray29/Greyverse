...
export default function Home(){
  ...
  const [email,setEmail]=useState(''),[password,setPassword]=useState(''),[username,setUsername]=useState(''),[game,setGame]=useState('eFootball'),[country,setCountry]=useState('')
  ...
  return <main className="shell">
    ...
    {section==='signup'&&<section className="hero narrow"><p className="eyebrow">JOIN GREYVERSE</p><h1>Create your player</h1><p className="muted">Choose your game. GreyVerse will create your account, player profile and automatically place you into a random available lower league.</p><form className="card form" onSubmit={signup}><input placeholder="Username" value={username} onChange={e=>setUsername(e.target.value)} required minLength={3}/><select value={game} onChange={e=>setGame(e.target.value)}>{games.length?games.map(g=><option key={g.id} value={g.game_name}>{g.game_name}</option>):<><option>eFootball</option><option>DLS</option></>}</select><select aria-label="Country" value={country} onChange={e=>setCountry(e.target.value)} required><option value="" disabled>Select country</option>{COUNTRIES.map(name=><option key={name} value={name}>{name}</option>)}</select><input type="email" placeholder="Email" value={email} onChange={e=>setEmail(e.target.value)} required/><input type="password" placeholder="Password (6+ characters)" value={password} onChange={e=>setPassword(e.target.value)} minLength={6} required/><button className="button" disabled={loading}>{loading?'Creating player…':'Create account'}</button><p className="muted">{message}</p></form></section>}
